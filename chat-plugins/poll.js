...
class Poll {
	...
	generateVotes(num) {
		...
		let output = '<div class="infobox"><p style="margin: 2px 0 5px 0"><span style="border:1px solid #6A6;color:#484;border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> Poll-' + this.pollArray[num].pollNum + '</span> <strong style="font-size:11pt">' + this.getQuestionMarkup(num) + '</strong></p>';
	...
	generateResults(ended, option, num) {
		...
		let output = '<div class="infobox"><p style="margin: 2px 0 5px 0">' + icon + ' <strong style="font-size:11pt">' + this.getQuestionMarkup(num) + '</strong></p>';
	...
	updateFor(user) {
		for (let u in this.pollArray) {
			if (user.userid in this.pollArray[u].voters) {
				user.sendTo(this.room, '|uhtmlchange|poll' + this.pollArray[u].pollNum + '|' + this.generateResults(false, this.voters[user.userid]));
			}
		}
	}
