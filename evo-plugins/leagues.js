...
function logPoints(message) {
	if (!message) return;
	let file = Path.join(__dirname, '../logs/leaguepoints.txt');
...
Evo.getLeagueRank = getLeagueRank;
...
function leagueTourPoints(winner, runnerup, tourSize, room) {
	...
	if (winnerLeague && winnerPoints > 0) {
		...
		save();
		logPoints(winner + " got First place in a tournament in " + room.id);
		room.addRaw("<b>" + winner + " has won " + winnerPoints + (winnerPoints === 1 ? " point " : " points ") + " for " + Chat.escapeHTML(leagues[winnerLeague].name) + "</b>");
	}
	if (secondLeague && secondPoints > 0) {
		leagues[secondLeague].points += secondPoints;
		save();
		logPoints(runnerup + " got Second place in a tournament in " + room.id);
		room.addRaw("<b>" + runnerup + " has won " + secondPoints + (secondPoints === 1 ? " point " : " points ") + " for " + Chat.escapeHTML(leagues[secondLeague].name) + "</b>");
	}
}
Evo.leagueTourPoints = leagueTourPoints;
...
function isLvLBattle(p1, p2, id, status, types, score) {
	...
	if (Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenger && Rooms.global.LvL[leagueId].challenger === targetLeagueid || Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenging && Rooms.global.LvL[leagueId].challenging === targetLeagueid) {
		...
		if (room.lvl.mode === "normal") {
			...
			if ((!room.lvl.leagues[0].players.includes(p1) && !room.lvl.leagues[1].players.includes(p1)) && (!room.lvl.leagues[0].players.includes(p2) && !room.lvl.leagues[1].players.includes(p2))) return;
		}
		...
		if (status === 'start') {
			...
			lvlDisplay(room);
			room.add('|raw|<a href="/' + id + '">The League vs League battle between ' + Evo.nameColor(p1, true) + ' (' + Chat.escapeHTML(leagues[leagueId].name) + ') and ' +
				Evo.nameColor(p2, true) + ' (' + Chat.escapeHTML(leagues[targetLeagueid].name) + ') has begun.</a>').update();
			...
			if (room.lvl.mode === "normal") {
				...
				!(room.lvl.leagues[0].wins > room.lvl.leagues[1].wins && room.lvl.leagues[0].wins === Math.ceil(room.lvl.size / 4 + 0.5) || room.lvl.leagues[1].wins > room.lvl.leagues[0].wins && room.lvl.leagues[1].wins === Math.ceil(room.lvl.size / 4 + 0.5)) ? room.lvl.status[room.lvl.statusNumber] = true : room.lvl.status[room.lvl.statusNumber] = false;

				if (room.lvl.status[room.lvl.statusNumber]) {
					room.lvl.status[room.lvl.statusNumber] = 2;
					room.lvl.leagues[0].wins > room.lvl.leagues[1].wins && room.lvl.leagues[0].wins === Math.ceil(room.lvl.size / 4 + 0.5) || room.lvl.leagues[1].wins > room.lvl.leagues[0].wins && room.lvl.leagues[1].wins === Math.ceil(room.lvl.size / 4 + 0.5);
					lvlDisplay(room);
				} else {
					lvlDisplay(room);
					// end
					...
					save();
					logPoints(toId(winner) + " Won League vs League against " + loser + " for " + points + " points.");
				...
				let finished = true;
				for (let u = 0; u < room.lvl.status.length; u++) {
					if (room.lvl.status[u] !== 0 && room.lvl.status[u] !== 1) finished = false;
				}

				if (room.lvl.status[room.lvl.statusNumber]) {
					room.lvl.status[room.lvl.statusNumber] = 2;
					room.lvl.leagues[0].wins > room.lvl.leagues[1].wins && room.lvl.leagues[0].wins === Math.ceil(room.lvl.size / 4 + 0.5) || room.lvl.leagues[0].wins > room.lvl.leagues[0].wins && room.lvl.leagues[1].wins === Math.ceil(room.lvl.size / 4 + 0.5);
					lvlDisplay(room);
				} else {
					lvlDisplay(room);
					// end
			...
			lvlDisplay(room);
			room.add("|raw|The League vs League battle between " + Evo.nameColor(p1, true) + " and " + Evo.nameColor(p2, true) + " ended with a tie. They have to have a rematch.");
			room.update();
		}
	}
}
Evo.isLvLBattle = isLvLBattle;

function lvlDisplay(room) {
	let output = '';
	output += '<center><font size="6">League vs League</font><br />';
	output += '<font color="grey"><small>(experimental - report any bugs to Volco... RIP!)</small></font>';
	...
	if (room.lvl.leagues[0].players.length === room.lvl.size && room.lvl.leagues[1].players.length === room.lvl.size && !room.lvl.started) {
		let notOnline = [];
		for (let u = 0; u < room.lvl.leagues[0].players.length; u++) {
		...
		for (let u = 0; u < room.lvl.leagues[1].players.length; u++) {
...
exports.commands = {
	...
	league: {
		...
		invite: function (target, room, user) {
			...
			leaguePM(user.name + " has invited " + targetUser.name + " to join the league.", leagueid);
		...
		accept: function (target, room, user) {
			...
			leaguePM(user.name + " has accepted their invitation to join the league.", leagueid);
		...
		decline: function (target, room, user) {
			...
			leaguePM(user.name + " has declined their invitation to join the league.", leagueid);
		...
		kick: function (target, room, user) {
			...
			leaguePM(user.name + " has kicked " + target + " from the league.", leagueid);

			if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + user.name + " has kicked you from the league " + Chat.escapeHTML(leagues[leagueid].name) + ".");
		...
		leave: function (target, room, user) {
			...
			leaguePM(user.name + " has left the league.", leagueid);
		...
		desc: function (target, room, user) {
			...
			leaguePM(user.name + " has set the league description to '" + Chat.escapeHTML(target) + "'.", leagueid);
		...
		pm: function (target, room, user) {
			...
			leaguePM("League announcement from " + user.name + ":<br />" + Chat.escapeHTML(target), leagueid);
		},
		...
		members: function (target, room, user) {
			...
			let sortedRanks = Object.keys(leagues[target].ranks).sort(function (a, b) {
				return leagues[target].ranks[b].sortBy - leagues[target].ranks[a].sortBy;
				
			});
			for (let rank = 0; rank < sortedRanks.length; rank++) {
				let users = [];
				...
				for (let u = 0; u < leagues[target].ranks[curRank].users.length; u++) {
		...
		list: function (target, room, user) {
			...
			for (let league = 0; league < sortedLeagues.length; league++) {
		...
		rank: {
			...
			give: function (target, room, user) {
				...
				leaguePM(user.name + " has set " + targetUser.name + "'s rank to " + Chat.escapeHTML(rank), leagueid);
				targetUser.send("|popup||html|" + user.name + " has set your league rank in " + Chat.escapeHTML(leagues[leagueid].name) + " to " +
			...
			take: function (target, room, user) {
				...
				leaguePM(user.name + " has removed the rank " + Chat.escapeHTML(rank) + " from " + targetUser, leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + user.name + " has removed you from the league rank " + Chat.escapeHTML(rank) + " in " +
			...
			create: function (target, room, user) {
				...
				leaguePM(user.name + " has added the rank '" + Chat.escapeHTML(rank) + "'.", leagueid);
			...
			delete: function (target, room, user) {
				...
				leagueLog(user.name + " has deleted the rank '" + Chat.escapeHTML(rank) + "'.", leagueid);
		...
		registerteam: function (target, room, user) {
			...
			for (let u = 0; u < splitTarget.length; u++) splitTarget[u] = splitTarget[u].trim();
			...
			getTeam(pastebin, (err, team) => {
				...
				leaguePM(user.name + " has registered their team to become a challenger.", toId(league));
		...
		resetteam: function (target, room, user) {
			...
			for (let rank in leagues[leagueid].ranks) {
				...
				if (leagues[leagueid].ranks['owner'].users.includes(user.userid) || leagues[leagueid].ranks['champion'].users.includes(user.userid) || leagues[leagueid].ranks['elitefour'].users.includes(user.userid)) can = true;
			}
		...
		challenge: function (target, room, user, connection) {
			...
			user.prepBattle(Tools.getFormat('gen7pokebankubers').id, 'challenge', connection).then(result => {
				if (result) {
					user.makeChallenge(targetUser, 'gen7monotype');
					targetUser.send('|pm|' + user.getIdentity() + '|~|/raw <div class="infobox">' + user.name + ' is challenging your league.</div>');
					user.send('|pm|' + targetUser.getIdentity() + '|~|/raw <div class="infobox">You are challenging ' + targetUser.name + '\'s league.');
					leaguePM(user.name + " has challenged " + targetUser.name + ".");
				}
			});
		},
		...
		badge: {
			add: function (target, room, user) {
				...
				leaguePM(user.name + " has added the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			edit: function (target, room, user) {
				...
				leaguePM(user.name + " has edited the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			delete: function (target, room, user) {
				...
				leaguePM(user.name + " has deleted the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			give: function (target, room, user) {
				...
				leaguePM(user.name + " has given the badge '" + Chat.escapeHTML(badgeName) + "' to " + targetUser + ".", leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + user.name + " has given you the league badge " + Chat.escapeHTML(badgeName) + " in " + Chat.escapeHTML(leagues[leagueid].name) + ".");
				}
			...
			take: function (target, room, user) {
				...
				leaguePM(user.name + " has taken the badge '" + Chat.escapeHTML(badgeName) + "' from " + targetUser + ".", leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + user.name +" has taken the league badge " + Chat.escapeHTML(badgeName) + " from you in " + Chat.escapeHTML(leagues[leagueid].name) + ".");
				}
			...
			view: function (target, room, user) {
				...
				if (!this.runBroadcast()) return;
				let badges = Evo.getBadges(target);
				if (badges.length < 1) return this.sendReplyBox(target + " has no league badges.");
				let output = target + "'s league badges:<br /><br />";
				for (let u in badges) {
					output += Chat.escapeHTML(u) + ":<br />";
					for (let i = 0; i < badges[u].length; i++)
		...
		lvl: {
			accept: function (target, room, user) {
				...
				leaguePM(user.name + ' has accepted the League vs League challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(user.name + ' (' + leagues[leagueId].name + ') has accepted the League vs League challenge against your league.', targetLeagueid);
			...
			deny: function (target, room, user) {
				...
				leaguePM(user.name + ' has declined the League vs League challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(user.name + ' (' + leagues[leagueId].name + ') has declined the League vs League challenge against your league.', targetLeagueid);
				...
				leaguePM(user.name + ' has forcibly ended the League vs League with ' + Chat.escapeHTML(leagues[targetLeagueid].name) + '.', leagueId);
			...
			invite: function (target, room, user) {
				...
				leaguePM(user.name + " has invited " + targetUser.name + " to join the League vs League against " + Chat.escapeHTML(leagues[targetLeague.id].name), leagueId);
				targetUser.send("|popup||modal||html|" + user.name + " has invited you to join the League vs League against " + Chat.escapeHTML(leagues[targetLeague.id].name) +
			...
			challenge: function (target, room, user) {
				...
				for (let i = 0; i < size.length; i++) room.lvl.status.push((mode === "normal" ? 3 : 2));

				leaguePM(
					user.name + ' (' + Chat.escapeHTML(getLeague(user.userid)) + ') has challenged your league to a League vs League (' +
					size + 'v' + size + ') in' +
				...
				leaguePM(
					user.name + ' has challenged ' + Chat.escapeHTML(leagues[targetLeagueid].name) + ' to a League vs League (' +
					size + 'v' + size + ') in <button name="joinRoom" value="' + room.id + '">' + Chat.escapeHTML(room.title) + '</button>'
				);
				room.add('|uhtml|lvl-' + lvlId + '|' +
					'<div class="infobox"><center>' + user.name + ' has challenged ' + Chat.escapeHTML(leagues[targetLeagueid].name) +
		...
		points: {
			give: function (target, room, user) {
				...
				Db('leaguepoints').set(league, Db('leaguepoints').get(league, 0) + amount).get(league);
				logPoints(league + " was given " + amount + " points by " + " admin " + user.name);
				this.sendReply("You've given " + leagues[league].name + " " + amount + (amount === 1 ? " point." : " points."));
				leaguePM(user.name + " has given your league " + amount + (amount === 1 ? " point." : " points."), league);
			},
			...
			take: function (target, room, user) {
				...
				Db('leaguepoints').set(league, Db('leaguepoints').get(league, 0) - amount).get(league);
				logPoints(league + " had taken " + amount + " points by " + " admin" + user.name);
				this.sendReply("You've taken " + amount + (amount === 1 ? " point " : " points ") + " from " + leagues[league].name + ".");
				leaguePM(user.name + " has taken " + amount + (amount === 1 ? " point " : " points ") + " from your league.", league);
			},
			...
			reset: function (target, room, user) {
				...
				//Evo.messageSeniorStaff("/html " + user.name + " has reset all league points.");
				Rooms('upperstaff').add("|raw|" + user.name + " has reset all league points.").update();
				delete user.confirmLeaguePointsReset;
				let leaguePoints = Db('leaguepoints').object();
				Object.keys(leaguePoints)
				.filter(function (name) {
					return Db('leaguepoints').get(name);
				})
				.forEach(function (name) {
					delete leaguePoints[name];
					});
				Db.save();
			},
...
try {
	...
	if (staff) staff.add(`|html|<div class="broadcast-red"><b>CUSTOM LEAGUE FUNCTIONALITY HAS CRASHED:</b><br />${e.stack}<br /><br /><b>Please report this to a developer.`).update();
}